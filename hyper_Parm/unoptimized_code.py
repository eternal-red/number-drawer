    ...
    def update_params(self, layer):
        ...
        if self.momentum:
            ...
            if not hasattr(layer, 'weight_momentums'):
                layer.weight_momentums = np.zeros(shape=layer.weights.shape)
                # If there is no momentum array for weights
                # The array doesn't exist for biases yet either.
                layer.bias_momentums = np.zeros(shape=layer.biases.shape)
...
# Add layers
model.add(Layer_Dense(input_dim[0] * input_dim[1], int(params['neurons1']), weight_regularizer_l2=params['l2_weight'], bias_regularizer_l2=params['l2_bias']))
...
model.train(x, y, validation_data=(x_test, y_test), #! based on CLT
            epochs=40, batch_size=int(params['batch_size']), print_every=1)
model.save("data/numDrawer.model")
